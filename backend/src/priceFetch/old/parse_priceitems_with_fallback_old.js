// full_reload_price_importer.js
// Clustered, streaming full reload: pure insert into temp coll + atomic swap
...
async function master() {
  ...
  const xmlPaths = await collectXmlFiles(XML_DIR);
  if (!xmlPaths.length) {
    console.error('No XML files found');
    process.exit(1);
  }
  console.log(`📦 Found ${xmlPaths.length} XML files. Forking ${NODES} workers`);
  ...
  console.log(`🏁 All workers done, inserted ${totalInserted} docs into ${TEMP_COLL}`);
...
async function worker() {
  const client = await new MongoClient(MONGO_URI).connect();
  const db = client.db();
  const coll = db.collection(TEMP_COLL);
  ...
  const chainMap = new Map(chains.map(c => [c.chainId, { chainName: c.chainName }]));

  const stores = await db.collection(STORE_COLL)
    .find().project({ _id:1, chainRef:1, subChainId:1, storeId:1 }).toArray();
  const storeMap = new Map(stores.map(s => [`${s.chainRef}_${s.subChainId}_${s.storeId}`, s._id]));
  ...
  const flush = () => {
    ...
    const p = coll.insertMany(docs, WRITE_OPTS)
  ...
  for (const filePath of slice) {
    ...
    const m = fileName.match(/^PriceFull(\d+)-(\d+)-/);
    if (!m) continue;
    const [ , chainIdRaw, storeIdRaw ] = m;
    const chainInfo = chainMap.get(chainIdRaw);
    if (!chainInfo) continue;

        // resolve storeRef with fallback, mirroring original script
    let subChainId = '1';
    ...
    const pfId = up.upsertedId
      ? up.upsertedId._id
      : (await pfColl.findOne({ storeRef, fileName }, { projection: { _id:1 } }))._id;
      ...
      parser.on('endElement', name => {
        if (name.toLowerCase() === 'item' && curr) {
          batch.push({
            ...
            priceUpdateDate: curr.PriceUpdateDate ? new Date(curr.PriceUpdateDate) : curr.PriceUpdateTime ? new Date(curr.PriceUpdateTime) : null,
            lastSaleDateTime: curr.LastSaleDateTime ? new Date(curr.LastSaleDateTime) : null,
            ...
            isWeighted: ['1','true'].includes(String(curr.BisWeighted || curr.bIsWeighted || '').toLowerCase()),
            qtyInPackage: parseFloat(curr.QtyInPackage) || 0,
            itemPrice: parseFloat(curr.ItemPrice) || 0,
            unitOfMeasurePrice: parseFloat(curr.UnitOfMeasurePrice || curr.UnitMeasurePrice) || 0,
            allowDiscount: ['1','true'].includes(String(curr.AllowDiscount || '').toLowerCase()),
...
if (cluster.isMaster) master(); else worker();
