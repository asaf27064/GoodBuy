// parse.js (or whatever you call it)

// Clustered, streaming full reload (init) or delta update (update)
...
async function master() {
  ...
  // in init mode: swap temp → live and rebuild indexes
  if (MODE === 'init') {
...
async function worker() {
  ...
  const storeMap = new Map(stores.map(s =>
    [`${s.chainRef}_${s.subChainId}_${s.storeId}`, s._id]
  ));
    ...
    await new Promise(resolve => {
      ...
      parser.on('startElement', name => {
        if (name.toLowerCase() === 'item') {
          curr = { priceFile: pfId, chainId: chainIdRaw, chainName: chainInfo.chainName };
        } else if (curr) {
          tag = name;
        }
      });
