...
function PriceComparisonScreen({route}) {
    ...
    const searchNearestStores = async function() {
        ...
        try {
            const response = await axios.get('http://192.168.0.105:3000/api/Stores/store_search', {params: {latitude: userLocation.coords.latitude, longitude: userLocation.coords.longitude}});
    ...
    const renderItem = ({item}) => {
      return (
        <View><Text style={{textAlign: 'right'}}>{item.store.storeName}</Text></View>
      );
    }

    const priceComparator = async function() {

      const storesIds = nearestStores.map(item => item.store._id.$oid);
      ...
      // Demo for price comparison algorithm. Return the store with cheapest list, two stores with combination of cheapest items, and closest store
      ...
      try {
        const response = await axios.get('http://192.168.0.105:3000/api/Products/list_price',
         {params: {stores: JSON.stringify(storesIds), products: JSON.stringify(productIdsAndAmount)}});
    ...
    return (
        <SafeAreaView style={{flex:1, backgroundColor: 'white'}}>
            <Button title="Calculate Nearest Stores" onPress={() => { searchNearestStores() }} />
            <Text>Location:</Text>
      <Text>
        {userLocation
      ...
      <Button title="Calculate best price" onPress={() => { priceComparator() }} />
      </SafeAreaView>
    );
}
